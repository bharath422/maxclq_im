...
\small
\centering
\caption{Results of communities detected using our new method and CFinder on LFR benchmarks \cite{2008PhRvE..78d6110L}. 
All networks have $n$ = 1000 nodes, and the parameters used to generate the netwroks are listed in the first three columns: $\mu$ is the mixing parameter, $O_m$ is the number of communities each overlapping node is assigned to $O_n$ is the fraction of overlapping nodes. $C$ denotes the number of communities, $S$ the number of shared nodes, and $\Omega$ the omega index. $C(GT)$ is the number of communities in the input graph (ground truth). }
...
\toprule\toprule
%	&  			& 			& 			&  		& 	CFinder	&			&		&Our Method	&			&		&	Our Method&			&		&Our Method	&		\\
